   ...
   **********************************************************************************************************************************
    ...
    Version History: 0.1.0.0 - Initial Release.
                     ...
                     0.1.6.0 - Updated 'Stop-PMESetup' function to colour code status if running interactively.
                               ...
                               'C:\ProgramData\Advanced Monitoring Agent\Repair-PME\' instead of default location.
                ...
                0.1.7.0 Beta - New function 'Set-PMEConfig' to apply fix for NCPM-4407 (System.OutOfMemoryException) this
                     ...
                     0.1.7.3 - New function 'Get-NCAgentVersion' which checks and provides information on currently installed
                               RMM Agent. If agent is not installed or incompatible with PME script will be aborted.
...
Function Get-PMELocations {
    ...
    # > PME Version 2.0
    If (Test-Path -Path "$ProgramFiles\Advanced Monitoring Agent\patchman\unins000.exe") {
        $PMEAgentUninstall = "$ProgramFiles\Advanced Monitoring Agent\patchman\unins000.exe"
        If (Test-Path -Path "$env:ProgramData\Advanced Monitoring Agent\patchman\archives") {
            $PMEArchives = "$env:ProgramData\Advanced Monitoring Agent\patchman\archives"
        }
        If (Test-Path -Path "$env:ProgramData\Advanced Monitoring Agent") {
            $PMEProgramDataPath = "$env:ProgramData\Advanced Monitoring Agent"
        }
    }
    If (Test-Path -Path "$ProgramFiles\Advanced Monitoring Agent\RequestHandlerAgent\unins000.exe") {
        $PMERPCUninstall = "$ProgramFiles\Advanced Monitoring Agent\RequestHandlerAgent\unins000.exe"
    }
    If (Test-Path -Path "$ProgramFiles\Advanced Monitoring Agent\FileCacheServiceAgent\unins000.exe") {
        $PMECacheUninstall = "$ProgramFiles\Advanced Monitoring Agent\FileCacheServiceAgent\unins000.exe"
    ...
    If (Test-Path -Path "$ProgramFiles\Advanced Monitoring Agent\RpcServer\unins000.exe") {
        $PMERPCUninstall = "$ProgramFiles\Advanced Monitoring Agent\RpcServer\unins000.exe"
    }
    If (Test-Path -Path "$ProgramFiles\Advanced Monitoring Agent\CacheService\unins000.exe") {
        $PMECacheUninstall = "$ProgramFiles\Advanced Monitoring Agent\CacheService\unins000.exe"
...
Function Get-NCAgentVersion {
    # Check if RMM Agent is currently installed
    Write-Host "Checking if RMM Agent is already installed..." -ForegroundColor Cyan
    $IsNCAgentInstalled = ""
        ...
        If ($null -ne $installed) {
            ForEach ($app in $installed) {
                If ($($app.DisplayName) -eq "Windows Agent" -and $($app.Publisher) -eq "N-able Technologies") {
                    ...
                    Write-Host "RMM Agent Installed: Yes" -ForegroundColor Green
                    Write-Output "RMM Agent Version: $($app.DisplayVersion)"
                    Write-Output "RMM Agent Install Date: $InstallDateFormatted"
                    If ($($app.DisplayVersion) -ge "12.2.0.274") {
                        Write-Host "RMM Agent PME Compatible: Yes" -ForegroundColor Green
                    } 
                    Else {
                        Write-Host "RMM Agent PME Compatible: No" -ForegroundColor Red
                        Write-EventLog @WriteEventLogErrorParams -Message "Installed RMM Agent ($($app.DisplayVersion)) is not compatible with PME, aborting.`nScript: Repair-PME.ps1"
                        Throw "Installed RMM Agent ($($app.DisplayVersion)) is not compatible with PME, aborting."
                    }
                }
                Else {
                    $IsNCAgentInstalled = "No"
                    Write-Host "RMM Agent Installed: No" -ForegroundColor Red
                    Write-EventLog @WriteEventLogErrorParams -Message "RMM Agent is not installed, PME requires an agent, aborting.`nScript: Repair-PME.ps1"
                    Throw "RMM Agent is not installed, PME requires an agent, aborting."
                }
            }
        }
    }
...
Function Clear-RepairPME {
    ...
    $RepairPMEPaths = "C:\ProgramData\Advanced Monitoring Agent\Repair-PME", "C:\ProgramData\Advanced Monitoring Agent\Repair-PME"
...
Function Invoke-PMEDiagnostics {
    ...
    If (Test-Path -Path "$ProgramFiles\Advanced Monitoring Agent\patchman\Diagnostics") {
        $PMEDiagnosticsFolderPath = "$ProgramFiles\Advanced Monitoring Agent\patchman\Diagnostics"
        $PMEDiagnosticsExePath = "$PMEDiagnosticsFolderPath\PME.Diagnostics.exe"
        $RepairPMEDiagnosticsLogsPath = "$env:ProgramData\Advanced Monitoring Agent\Repair-PME\Diagnostic Logs"
        $ZipPath = "/`"ProgramData/Advanced Monitoring Agent/Repair-PME/Diagnostic Logs/PMEDiagnostics$(Get-Date -Format 'yyyyMMdd-hhmmss').zip`""
    }
    ElseIf (Test-Path -Path "$ProgramFiles\Advanced Monitoring Agent\patchman\Diagnostics") {
        $PMEDiagnosticsFolderPath = "$ProgramFiles\Advanced Monitoring Agent\patchman\Diagnostics"
...
Function Clear-PME {
    ...
    $CacheFolderPaths = "$env:ProgramData\Advanced Monitoring Agent\SolarWinds.MSP.CacheService", "$env:ProgramData\Advanced Monitoring Agent\SolarWinds.MSP.CacheService\cache", "$env:ProgramData\Advanced Monitoring Agent\FileCacheServiceAgent", "$env:ProgramData\Advanced Monitoring Agent\FileCacheServiceAgent\cache"
